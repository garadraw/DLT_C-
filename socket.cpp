...
// bool Socket::_active = true;

Socket::Socket(string serverPass, int port): fd_server(), clients(), msg(), handler(), _serverName("OurServer"), _channelList(), _port(port), _pass(serverPass)
{
	std::vector<User *> _userList;
	std::cout << "Socket Object Created" << std::endl;
}
...
void	Socket::setPass(std::string pass) {
	this->_pass = pass;
}

std::string	Socket::getPass() const {
	return (this->_pass);
}
	...
	for (int i = 0; i < 1024; i++)
	{
		if ((this->clients[i].revents & POLLIN) == POLLIN) // fd is ready for reading
		{
			...
			if (this->clients[i].fd == this->fd_server) // request for new connection
			{
				...
				{
					std::cout << "Socket Capacity for Clients is Full" << std::endl;
					close(userSocket);
				}
			}
...
bool	Socket::free_everything() {
	...
	std::cout << "free_everything(): All users killed. Socket ready to shut down." << std::endl;
...
User*	Socket::findByFd(int clientFd) {
...
User*	Socket::findByNick(std::string nick) {
...
Channel*	Socket::findChannel(std::string name) {
...
void	Socket::killUser(User* user)
{
